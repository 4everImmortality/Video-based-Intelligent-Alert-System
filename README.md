# 视频告警系统
这个系统是我的本科毕业设计，主要是包含了web系统、目标检测算法、大模型聊天界面设计等方面，后面有需要的同学，如果有同学用来做毕业设计，那么在web系统不需要在重新写了，只需要把自己构思目标检测算法层面或者业务层面的改进创新（我这里用的是Ultralytics）都可以在**media**目录中轻松的部署，再在**Admin**中注册算法。如果有需要做大模型相关的改进，可以自己构建一个Restful 服务器，在**Admin下的ai_chat_google**中对应修改。
## 系统架构
